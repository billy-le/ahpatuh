...
const navigation: Array<Pick<LinkComponentProps, 'to'> & { name: string, icon: React.ReactElement }> = [
  {
    to: '/dashboard',
    name: 'Dashboard',
...
export function Layout({ children }: React.PropsWithChildren) {
  ...
  return <div className='flex gap-4 min-h-dvh'>
        ...
        <ul className='space-y-2'>
          {navigation.map(({ name, to, icon }) =>
            <li className='rounded hover:bg-yellow-400/20' key={name}>
              <Link activeProps={{ className: 'block bg-yellow-300 rounded-md' }} to={to}>
                <div className='flex items-center gap-4 py-2 px-4'>{icon}<span>{name}</span></div>
              </Link>
            </li>
          )}
        </ul>
